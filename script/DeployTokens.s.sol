    ...
    function run() external {
        ...
        for (uint256 i = 0; i < chainRPCs.length; i++) {
            ...
            for (uint256 j = 0; j < tokenNames.length; j++) {
                ...
                string memory tokenName = string.concat(chainSymbols[i], " ", tokenNames[j]);
                string memory tokenSymbol = string.concat(chainSymbols[i], "-", tokenSymbols[j]);
                ...
                string memory message = string.concat(
                    "Deployed [", chainSymbols[i], " ", tokenNames[j], "] at: ", Strings.toHexString(address(token))
                );
                console.log(message);
            }
        }
